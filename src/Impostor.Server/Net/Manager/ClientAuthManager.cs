...
public class ClientAuthManager(ILogger<ClientAuthManager> logger)
{
    ...
    {
        var id = GetNextId();
        if (TryGetAuthInfo(n => n.MatchmakerToken == matchmakerToken || n.FriendCode == friendCode, info: out var authInfo))
        {
            authInfo.LastId = id;
        ...
        AuthInfos.Add(info);
        logger.LogInformation("Create authInfo:{id} {version}, {platform}, {token}, {code}", id, version, platform, matchmakerToken, friendCode);
        return id;
    }
    ...
    public bool TryGetAuthInfo(uint id, [MaybeNullWhen(false)] out AuthInfo info) => TryGetAuthInfo(n => n.LastId == LastId, out info);
}
