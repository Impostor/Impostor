...
internal partial class InnerPlayerControl : InnerNetObject
{
    ...
    private async ValueTask<bool> HandleCheckName(ClientPlayer sender, string name)
    {
        ...
        {
            if (await sender.Client.ReportCheatAsync(RpcCalls.CheckName,
    ...
    private async ValueTask<bool> HandleSetName(ClientPlayer sender, string name)
    {
        ...
        if (sender.IsOwner(this))
        {
            ...
            if (sender.Client.Name != name)
            {
                if (await sender.Client.ReportCheatAsync(RpcCalls.SetName,
                        "Client sent name not matching his name from handshake"))
                {
                    return false;
                }
    ...
    private async ValueTask<bool> HandleSetColor(ClientPlayer sender, ColorType color)
    {
        ...
        {
            if (Game.Players.Any(x =>
                    x.Character != null && x.Character != this && x.Character.PlayerInfo.CurrentOutfit.Color == color))
            {
        ...
        else
        {
            ...
            while (Game.Players.Any(x =>
                       x.Character != null && x.Character != this &&
                       x.Character.PlayerInfo.CurrentOutfit.Color == expected))
            {
    ...
    private async ValueTask<bool> HandleProtectPlayer(ClientPlayer sender, IInnerPlayerControl? target)
    {
        ...
        {
            if (await sender.Client.ReportCheatAsync(RpcCalls.CheckProtect,
                    "Client tried to protect but it wasn't a guardian angel"))
            {
                return false;
            }
        }
    ...
    private async ValueTask<bool> HandleSetStartCounter(ClientPlayer sender, int sequenceId, sbyte startCounter)
    {
        ...
        {
            if (await sender.Client.ReportCheatAsync(RpcCalls.MurderPlayer,
