...
namespace Impostor.Server.Recorder
{
    internal class ClientRecorder : Client
    {
        ...
        public ClientRecorder(ILogger<Client> logger, IOptions<AntiCheatConfig> antiCheatOptions, IOptions<ModConfig> modConfigOptions, ClientManager clientManager, ICustomMessageManager<ICustomRootMessage> customMessageManager, GameManager gameManager, string name, int gameVersion, Language language, QuickChatModes chatMode, PlatformSpecificData platformSpecificData, HazelConnection connection, PacketRecorder recorder)
            : base(logger, antiCheatOptions, modConfigOptions, clientManager, gameManager, customMessageManager, name, gameVersion, language, chatMode, platformSpecificData, connection)
        {
            _recorder = recorder;
            _isFirst = true;
            _createdGame = false;
